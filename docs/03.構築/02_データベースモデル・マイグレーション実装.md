...
- `uv run python manage.py startapp kakeibo`: `kakeibo` という名前の Django アプリを作成する
...
- `uv run python manage.py makemigrations kakeibo`: `kakeibo` アプリのモデル定義から
  マイグレーションファイルを生成する
...
- `docker compose up -d db`: ローカル開発用の MySQL コンテナ（`db` サービス）を
  バックグラウンドで起動する
- `uv run python manage.py migrate`: 生成したマイグレーションを DB に適用する
...
- `uvx ruff check .`: ruff でコードの静的解析（lint）を実行する
- `uvx ruff format --check .`: ruff でフォーマット崩れがないか確認する
