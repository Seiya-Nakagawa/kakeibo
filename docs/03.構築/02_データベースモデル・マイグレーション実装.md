...
docker compose exec db sh -c 'mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -e "SHOW CREATE TABLE kakeibo_transaction\G"'
```

- `db` コンテナ内で mysql クライアントを実行し、実際に作成されたテーブルの一覧を表示する
- `transaction` テーブルの DDL を表示し、カラム型・UNIQUE制約・インデックス・文字コード
  （`utf8mb4` / `utf8mb4_0900_ai_ci`）が[基本設計書 4章](../02.設計/基本設計書.md#4-データベース設計)
  どおりか確認する
