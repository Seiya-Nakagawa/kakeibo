...
| 認証情報の保管 | OCI Vault → External Secrets Operator → Kubernetes Secret（[6.1節](#61-gmail-api)の Gmail API 用認証情報とは別の OAuth クライアントを使用する） |
...
| 保管先 | OCI Object Storage（[Amazon S3 互換 API](https://docs.oracle.com/ja-jp/iaas/Content/Object/Tasks/s3compatibleapi.htm)、`boto3` で操作。OCI SDK 固有の認証情報（テナンシ OCID 等）を増やさず、アクセスキー・シークレットキーの2値のみで完結するため採用） |
