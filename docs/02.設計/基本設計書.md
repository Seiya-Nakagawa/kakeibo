...
> **本書の状態**: 第 2 版。Pod から MySQL への接続方式、データモデル、画面設計を追記した。
> カテゴリ自動学習のキーワード抽出規則および通知方式は、第 8 章の未決事項として引き続き検討する。
...
> **未決**: 店舗名からどの部分をキーワードとして抽出するかの規則は、実データを確認しながら決定する
> （第 8 章 #3）。
...
| Pod からの接続方式 | DB へ接続する Pod（Web、CronJob）は `hostNetwork: true` で稼働し、ホストの `127.0.0.1:3306` へ直接接続する |

#### 4.1.1. Pod から MySQL への接続方式（第 8 章の旧 #1 を決定）
...
| 選択肢 | 採用 | 理由 |
| ------ | ---- | ---- |
| Web Pod・CronJob Pod を `hostNetwork: true` で稼働させる | **採用** | MySQL の待受設定を一切変更せずに、Pod からホストの `127.0.0.1` へ到達できる。DB は引き続きループバックのみで待ち受け、Pod ネットワーク・外部ネットワークいずれにも公開されない。本クラスタの `ingress-nginx` で既に採用されている手法であり、追加の運用パターンを増やさない |
...
- `hostNetwork: true` の Pod はホストのポート空間を直接使用するため、アプリケーションの待受ポートは
  `ingress-nginx`（80/443）や `mysqld`（3306）と重複しないものを選定する（Web Pod は 8000 番台を使用する）
- DB への接続元は、MySQL のユーザー権限（専用ユーザーに当該スキーマのみ付与）でも制限する
...
| 項目 | 内容 |
| ---- | ---- |
| 用途 | 決済通知メールの取得、ラベルの付け替え |
| クライアント | google-api-python-client |
| 認証方式 | OAuth 2.0。認証情報の維持方式は未決（第 8 章 #1） |
...
| DB 公開範囲 | Web Pod・CronJob Pod を `hostNetwork: true` で稼働させ、ホストの `127.0.0.1` へのみ接続する。ネットワーク経由での外部公開は行わない（詳細は 4.1.1 節） |
...
| 通知 | 未決（第 8 章 #4）。通知手段（メール送信経路等）は決定次第追記する |

## 8. 未決事項

[要件定義書 第8章](../01.要件定義/要件定義書.md)から引き継いだ、本設計フェーズで決定すべき論点。

> Pod から MySQL への接続方式は 4.1.1 節のとおり決定済みのため、本表から除外した。

| # | 論点 | 検討状況 |
| - | ---- | -------- |
| 1 | Gmail API の認証情報の維持方式 | 第一候補は OAuth 同意画面の本番公開。代替案はサービスアカウント + ドメイン全体の委任 |
| 2 | 決済手段と口座の紐づけ方針 | 取引の計上は発生主義（利用日基準）、資産残高は手動記録による実残高とする方針を前提とする。両者の差の表示方法が未決 |
| 3 | カテゴリ自動学習時のキーワード抽出規則 | 未決。実データを確認しながら定める |
| 4 | 通知手段 | 未決。エラー通知の送信経路を決定する |
