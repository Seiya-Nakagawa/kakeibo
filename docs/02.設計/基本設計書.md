...
| 機密情報 | `.env` を入力に `scripts/create_secret.sh` で Kubernetes Secret（`kakeibo-secrets`）を手動生成 |
...
Kubernetes Secret（`kakeibo-secrets`）はデプロイ担当者が `webapp/.env.production` から
`scripts/create_secret.sh` を実行して事前に作成しておくものであり、
クラスタ内で稼働する常駐コンポーネントではないため上図には含めない。
`webapp/.env.production` とローカル開発用の `webapp/.env` の役割の違いは
[7.3節](#73-セキュリティ要件-63)を参照。
...
| 認証情報の保管 | `webapp/.env.production` から `scripts/create_secret.sh` で作成した Kubernetes Secret（`kakeibo-secrets`） |
...
| 認証情報の保管 | `webapp/.env.production` から `scripts/create_secret.sh` で作成した Kubernetes Secret（`kakeibo-secrets`。[6.1節](#61-gmail-api)の Gmail API 用認証情報とは別の OAuth クライアントを使用する） |
...
`webapp/.env.production`（Git 管理外）にのみ保持し、`scripts/create_secret.sh` で
Kubernetes Secret（`kakeibo-secrets`）へ手動反映する。ソースコードおよびコンテナイメージには
埋め込まない。

`webapp/.env` と `webapp/.env.production` はいずれも Git 管理外の環境変数ファイルだが、
役割が異なる。

| ファイル | 役割 |
| -------- | ---- |
| `webapp/.env` | ローカル開発用。`development.py` での動作確認（`manage.py runserver`・テスト実行等）に使用する |
| `webapp/.env.production` | 本番 Kubernetes Secret 生成専用。`production.py` 専用の追加設定（`ALLOWED_HOSTS`・`DB_SOCKET_PATH`・`FORCE_SCRIPT_NAME`）を含み、`scripts/create_secret.sh` が引数省略時にデフォルトで読み込む入力ファイルである |

本番の認証情報をローテーションする際は、`webapp/.env` だけでなく `webapp/.env.production` にも
同じ値を反映してから `scripts/create_secret.sh` を実行すること。`webapp/.env.production`側への
反映を怠ると、更新前の古い内容で本番 Secret を上書きしてしまう
（Issue #46・[Issue #59](https://github.com/Seiya-Nakagawa/kakeibo/issues/59)対応中に
実際に発生した障害）。
