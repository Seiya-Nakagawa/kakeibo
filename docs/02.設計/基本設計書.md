...
| ドメイン | `technohonesty.com`（個人開発の複数アプリを同一ドメイン配下でパスにより振り分ける。本システムは `/kakeibo` を割り当てる） |
| TLS | `cert-manager` + Let's Encrypt による証明書自動管理 |
| データベース | MySQL 8.0（ホスト OS に直接導入、`bind-address: 127.0.0.1`） |
| 機密情報 | OCI Vault（`ap-osaka-1`）を保管先とし、External Secrets Operator が Kubernetes Secret（`kakeibo-secrets`）へ同期 |
...
個人開発の複数アプリを、サブドメインではなく単一ドメイン配下のパスで振り分ける
（本システムは `https://technohonesty.com/kakeibo/` 配下）。

- `ingress-nginx` の Ingress リソースでパスプレフィックス（`/kakeibo`）ごとに
  バックエンドの Service を振り分ける
- Django 側はパスプレフィックスを付けずに実装し、`FORCE_SCRIPT_NAME`（本番設定）で
  `/kakeibo` を付与することで、URL 逆引き・リダイレクト先を含めて自アプリが
  `/kakeibo` 配下で動作していることを認識させる。Ingress 側はプレフィックスを
  除去してバックエンドへ転送し、除去前のプレフィックスを `X-Script-Name` 等の
  ヘッダーで伝搬する
- Google OAuth（[6.4節](#64-ログイン認証google-oauth要件-49)）のリダイレクトURIも
  `https://technohonesty.com/kakeibo/accounts/google/login/callback/` を本番用として登録する
- Ingress・`FORCE_SCRIPT_NAME` の具体的な設定は Kubernetes マニフェスト作成
  （[Issue #29](https://github.com/Seiya-Nakagawa/kakeibo/issues/29)）で実施する
...
graph TD
    ...
    subgraph OCI["OCI インスタンス（oci-portfolio）"]
        subgraph K8S["Kubernetes（シングルノード）"]
            ...
            ESO[External Secrets Operator<br/>kakeibo-secrets を生成]
        end
        MySQL[(MySQL 8.0<br/>ホスト OS)]
    end

    Vault[(OCI Vault<br/>シークレット)]

    User((家族ユーザー)) -->|HTTPS| Ingress
    Ingress --> Web
    Web --> MySQL
    CronImport -->|Gmail API| Gmail
    CronImport -->|エラー通知| Slack
    CronImport --> MySQL
    ESO -->|Instance Principal| Vault
    ESO -.->|Secret 同期| Web
    ESO -.->|Secret 同期| CronImport
```

Kubernetes Secret（`kakeibo-secrets`）は、External Secrets Operator が OCI Vault のシークレットから
生成・更新する（`k8s/vault-sync.yaml`、同期間隔 1 時間）。External Secrets Operator と OCI Vault への
接続定義（`ClusterSecretStore/oci-vault`、Instance Principal 認証）は `infra-oci` 側で管理する。
ローカル開発用の `webapp/.env` との役割の違いは[7.3節](#73-セキュリティ要件-63)を参照。

DBバックアップ（要件6.4）は本システム専用ではなく、ホスト OS 上で複数の個人開発
プロジェクトが共用する MySQL インスタンス全体を対象とするため、本システムの
Kubernetes マニフェストには含めない。実現方式は[7.2節](#72-バックアップ要件-64)を参照。
...
4. 未分類一覧画面で利用者がカテゴリを割り当てると、その店舗名から抽出したキーワードを
   ...
   キーワードは店舗名（正規化済み）全体とする。部分的なブランド名抽出（支店名の切り捨て等）は
   実データにおける表記ゆれの傾向が確認できていないため採用せず、同一店舗名での完全一致相当を
   自動学習の単位とする保守的な規則とする
...
- マウント対象はソケットファイル単体ではなく `/var/run/mysqld` ディレクトリとする
- ホストの `mysql.service` は `RuntimeDirectoryPreserve=yes`（`infra-oci-ansible` 側の
  systemd drop-in）を前提とする。既定の `no` のままでは `mysqld` 再起動のたびに
  `/var/run/mysqld` が systemd によってディレクトリごと再作成され、既存 Pod の
  `hostPath` マウントが古い実体を指したまま切断される。この設定変更後も、
  既に起動済みの Pod の `hostPath` は再作成前の実体を指し続けるため、設定変更の
  反映には対象 Pod の再作成（`kubectl rollout restart`）を要する
...
| google_sub | VARCHAR(255) | NULL, UNIQUE | Google OAuth の `sub`（ユーザー識別子）。事前登録時点では未確定のため、初回ログイン成功時に設定する |
| email | VARCHAR(255) | NOT NULL, UNIQUE | 事前登録（利用者管理画面）の一致キー |
| display_name | VARCHAR(100) | NOT NULL | |
| role | VARCHAR(20) | NOT NULL | `admin` / `general` |
| is_active | BOOLEAN | NOT NULL, DEFAULT TRUE | ログイン許可の有効フラグ |
| password | VARCHAR(255) | NOT NULL | Django 認証基盤（`AbstractBaseUser`）が要求する技術的カラム。Google OAuth のみでログインするため業務上は未使用とし、`set_unusable_password()` でログイン不能な値を設定する |
| last_login | DATETIME | NULL | Django 認証基盤が自動更新する最終ログイン日時 |
| created_at / updated_at | DATETIME | NOT NULL | |

利用者の事前登録は `email` を一致キーとして行うため、登録時点では `google_sub` は不明である。
初回の Google ログイン成功時に `sociallogin.account.uid` を `google_sub` へ設定することで、
以後は Google 側の `sub` によるユーザー識別も可能にする。
...
フォーム・一覧・管理画面の項目名はすべて日本語表記とする。モデルフィールドの `verbose_name`
（[4.2 節](#42-テーブル定義)）を単一の情報源とし、`ModelForm` 以外のフォームフィールドは
個別に日本語の `label` を設定する。
...
全画面共通で、日付入力欄（`type="date"`）は選択後にその右側へ曜日を括弧書きで表示する。
...
| 共通項目 | 日付（必須、新規登録時は当日をデフォルト表示）、金額（必須）、店舗名・収入元（必須）、カテゴリ、メモ |
| 支出項目 | 決済手段 |
| 収入項目 | 入金先口座 |
...
| 使用スコープ | `gmail.readonly`、`gmail.modify`、`gmail.send`（必要最小限に限定） |
| 認証情報の保管 | OCI Vault から External Secrets Operator 経由で同期した Kubernetes Secret（`kakeibo-secrets`） |
...
メール取込で登録する取引の `created_by`（登録者、要件4.9.5）は、対話的なログインユーザーが
存在しないため、環境変数 `MAIL_IMPORT_USER_EMAIL` で指定した `users.email` に一致する
利用者を登録者として記録する。
...
### 6.4. ログイン認証（Google OAuth、要件 4.9）

| 項目 | 内容 |
| ---- | ---- |
| 用途 | 家族向け画面（5.1.1節）へのログイン（利用者の本人確認のみ） |
| クライアント | django-allauth（`allauth.socialaccount.providers.google`） |
| 認証方式 | OAuth 2.0 / OpenID Connect（Authorization Code フロー） |
| 使用スコープ | `openid`、`profile`、`email`（本人確認に必要な最小限） |
| 認証情報の保管 | OCI Vault から External Secrets Operator 経由で同期した Kubernetes Secret（`kakeibo-secrets`。[6.1節](#61-gmail-api)の Gmail API 用認証情報とは別の OAuth クライアントを使用する） |
| 事前登録制御 | 未登録（`users.email` に一致するレコードがない、または `is_active = FALSE`）のアカウントはログインを拒否し、画面8（アクセス拒否）へ遷移する |
| 権限判定 | `users.role = admin` のアカウントのみ Django Admin（staff 権限）へアクセスできる。ロールベースの2段階権限であり、`django.contrib.auth` の Group/Permission 機構は使用しない |
| セッション | 無操作が一定時間（既定30分、環境変数で変更可）継続した場合にセッションを失効させ、再ログインを求める（要件 4.9.6） |

### 6.5. Slack Incoming Webhook（要件 6.5）

| 項目 | 内容 |
| ---- | ---- |
| 用途 | エラー通知の送信（Gmail API の認証障害時にも通知を届けるための、Gmail 以外の通知経路） |
| 送信方式 | Webhook URL への HTTP POST（JSON 形式）。標準ライブラリのみで実装し、追加ライブラリは導入しない |
| 通知先 | 管理者用の Slack チャンネル。Webhook URL 自体が送信権限を持つため、機密情報として扱う |
| 認証情報の保管 | OCI Vault から External Secrets Operator 経由で同期した Kubernetes Secret（`kakeibo-secrets`）。環境変数 `SLACK_WEBHOOK_URL` で参照する |

エラー通知は Gmail（[6.1節](#61-gmail-api)）と Slack の両経路に対して**それぞれ独立に**送信する。
一方の経路で失敗しても他方の送信は継続し、両方が失敗した場合のみログに記録する。
`SLACK_WEBHOOK_URL` が未設定の場合は Slack への送信を行わず、Gmail のみで通知する
（ローカル開発を想定）。
...
DB はホスト OS 上に複数の個人開発プロジェクトが共用する形で導入されており
（[4.1節](#41-共通方針)）、本システム固有の Kubernetes マニフェストで個別に
バックアップする構成は採らない。ホスト OS 側で全 DB を対象に一括してバックアップを
取得する方式とし、実現方式（取得方法・保管先・世代管理・失敗時通知）はインフラ側の
設計書で管理する。要件6.4（日次取得、失敗検知・通知、四半期毎のリストア検証、
無料枠内のコスト）は実現場所によらず満たす。

### 7.3. セキュリティ（要件 6.3）

認証情報（Django `SECRET_KEY`、Google OAuth クライアントシークレット、リフレッシュトークン、
DB パスワード等）は OCI Vault にのみ保管し、External Secrets Operator が Kubernetes Secret
（`kakeibo-secrets`）へ同期する。ソースコードおよびコンテナイメージには埋め込まない。
Web Pod・CronJob は従来どおり `envFrom` で Secret を環境変数として参照するため、
Django 側の実装は本番・ローカルで共通である。

| 項目 | 内容 |
| ---- | ---- |
| 保管先 | 基盤（`infra-oci`）が管理する OCI Vault（DEFAULT Vault、Always Free 枠内）。シークレット名は `kakeibo-` を接頭辞とする |
| 同期対象 | `SECRET_KEY`、`DB_NAME`、`DB_USER`、`DB_PASSWORD`、`GOOGLE_OAUTH_CLIENT_ID`、`GOOGLE_OAUTH_CLIENT_SECRET`、`GMAIL_API_CLIENT_ID`、`GMAIL_API_CLIENT_SECRET`、`GMAIL_API_REFRESH_TOKEN`、`NOTIFICATION_RECIPIENT_EMAIL`、`SLACK_WEBHOOK_URL`、`MAIL_IMPORT_USER_EMAIL`（12 件） |
| 同期方式 | `ExternalSecret`（`k8s/vault-sync.yaml`）が `ClusterSecretStore/oci-vault` を参照し、1 時間間隔で同期する。`deletionPolicy: Retain` により、ExternalSecret を削除しても稼働中の Pod が参照する Secret は残る |
| 認証 | External Secrets Operator が稼働するインスタンスの Instance Principal。Vault の読み取り権限は基盤側の Dynamic Group とポリシーで付与し、本システムはキー・パスワードを保持しない |
| ローカル開発 | `webapp/.env`（Git 管理外）を `django-environ` で読み込む。Vault は使用しない |
| 機密情報を含まない設定 | `ALLOWED_HOSTS`・`DB_SOCKET_PATH`・`FORCE_SCRIPT_NAME`・`SESSION_TIMEOUT_SECONDS` は ConfigMap（`k8s/configmap.yaml`）で供給する |

本番の認証情報をローテーションする際は、Vault のシークレットを新バージョンで更新する。
Secret への反映は次回の同期（最大 1 時間）で行われ、環境変数として読み込み済みの Web Pod は
再起動するまで旧値を保持する（CronJob は実行ごとに起動するため次回実行から新値になる）。
...
| TLS | ingress-nginx で終端し、cert-manager が証明書を自動更新する。ingress-nginx から Pod へは平文 HTTP で転送されるため、Django 側は `SECURE_PROXY_SSL_HEADER`（`HTTP_X_FORWARDED_PROTO`）を設定し、`request.is_secure()` が正しく `True` を返すようにする（未設定だと Google OAuth のコールバック URL が `http://` で生成され `redirect_uri_mismatch` になる） |
...
| デプロイ | コンテナイメージのビルドと Kubernetes マニフェストの適用により行う。イメージレジストリは使用せず、`docker save` \| `ssh` \| `nerdctl -n k8s.io load` でホストの containerd へ直接ロードする（`imagePullPolicy: IfNotPresent`） |
| Web Pod の死活監視 | `ALLOWED_HOSTS` を公開ドメインのみに限定しているため、readinessProbe は Host ヘッダーを付与した exec プローブ（コンテナ内から自身へ HTTP リクエスト）で判定する。livenessProbe は tcpSocket で判定する |
| スキーマ変更 | Django のマイグレーションで管理する |
| 通知 | Gmail API（`gmail.send` スコープ、[6.1節](#61-gmail-api)）による家族の代表メールアドレス宛のメール送信と、Slack Incoming Webhook（[6.5節](#65-slack-incoming-webhook要件-65)）による管理者用チャンネルへの投稿を併用する。SMTP 認証情報は新設しない |
