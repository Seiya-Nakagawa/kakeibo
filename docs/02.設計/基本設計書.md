...
Kubernetes Secret（`kakeibo-secrets`）はデプロイ担当者が `.env` から
...
| 認証情報の保管 | `.env` から `scripts/create_secret.sh` で作成した Kubernetes Secret（`kakeibo-secrets`） |
...
| 認証情報の保管 | `.env` から `scripts/create_secret.sh` で作成した Kubernetes Secret（`kakeibo-secrets`。[6.1節](#61-gmail-api)の Gmail API 用認証情報とは別の OAuth クライアントを使用する） |
...
`.env`（Git 管理外）にのみ保持し、`scripts/create_secret.sh` で Kubernetes Secret
（`kakeibo-secrets`）へ手動反映する。ソースコードおよびコンテナイメージには埋め込まない。
