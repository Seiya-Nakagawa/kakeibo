...
| TLS | `cert-manager` + Let's Encrypt による証明書自動管理（証明書は基盤側で管理する） |
| Namespace | 全サービス共有の `app-prod`（基盤側で管理する） |
...
- Namespace（`app-prod`）・Ingress・TLS 証明書は、全サービスで共有するため基盤側
  （`infra-oci`）で管理し、本システムのマニフェストには含めない。
  基盤側の共有 Ingress が、パスプレフィックス（`/kakeibo`）ごとに
  バックエンドの Service を振り分ける
- 本システムは Service `kakeibo-web`（ポート 80）を `app-prod` に公開する。
  基盤側のルーティング設定がこの Service 名・ポートを参照するため、変更しない
- 共有 Namespace 内で他サービスと名前が衝突しないよう、本システムの Kubernetes リソース
  （Deployment・Service・ConfigMap・Secret・CronJob 等）は `kakeibo-` プレフィックスを付ける
- 共有 Namespace のため、Secret・RBAC・NetworkPolicy のサービス間分離は行えない
...
- `FORCE_SCRIPT_NAME` の具体的な設定は Kubernetes マニフェスト作成
...
- 対象は DB へ接続する Web Pod と CronJob Pod（メール取込）のみとし、
...
メール取込は Django の管理コマンドとして実装し、CronJob から呼び出す。
DB バックアップは本システムのバッチではなく、ホスト OS 側で実施する（[7.2節](#72-バックアップ要件-64)）。
...
| 同期対象 | `SECRET_KEY`、`DB_NAME`、`DB_USER`、`DB_PASSWORD`、`GOOGLE_OAUTH_CLIENT_ID`、`GOOGLE_OAUTH_CLIENT_SECRET`、`GMAIL_API_CLIENT_ID`、`GMAIL_API_CLIENT_SECRET`、`GMAIL_API_REFRESH_TOKEN`、`NOTIFICATION_RECIPIENT_EMAIL`、`MAIL_IMPORT_USER_EMAIL`（11 件） |
