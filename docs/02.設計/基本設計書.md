...
| 保管先 | OCI Object Storage（[Amazon S3 互換 API](https://docs.oracle.com/ja-jp/iaas/Content/Object/Tasks/s3compatibleapi.htm)、`boto3` で操作。OCI SDK 固有の認証情報（テナンシ OCID 等）を増やさず、アクセスキー・シークレットキーの2値のみで完結するため採用。OCI Object Storage の S3 互換 API は botocore 1.36 以降が既定で行うチェックサム付き chunked encoding の PutObject に対応していないため、`Config(request_checksum_calculation="when_required", response_checksum_validation="when_required")` を指定して無効化する） |
| 世代管理 | 日次 30 世代、月次 12 世代（`backups/daily/`・`backups/monthly/` プレフィックスにオブジェクトキーの日付降順で保持し、超過分を削除） |
